...
public class MinioAsyncConfiguration {

    private static final Logger LOGGER = LoggerFactory.getLogger(MinioAsyncConfiguration.class);

    private final MinioConfigurationProperties minioConfigurationProperties;

    @Bean
    public MinioAsyncClient minioAsyncClient() throws IOException, InvalidKeyException, NoSuchAlgorithmException, InsufficientDataException, InternalException, ErrorResponseException, InvalidResponseException, MinioException, XmlParserException, ServerException, inc.yowyob.service.media.api.exceptions.MinioException, ExecutionException, InterruptedException {
        ...
        if (minioConfigurationProperties.isCheckBucket()) {
            try {
                ...
                boolean b = minioClient.bucketExists(existsArgs).get();
                ...
                if (!b) {
                    if (minioConfigurationProperties.isCreateBucket()) {
                        try {
                            ...
                            throw new inc.yowyob.service.media.api.exceptions.MinioException("Cannot create bucket", e);
                        }
                    } else {
